...
async function loadChangeHistory() {
    ...
    if (historyData.length === 0) {
        ...
        if (historyTableBody.parentElement) {
            historyTableBody.parentElement.classList.add('hidden'); // Ocultar <table>
        }
    } else {
        historyEmptyState.classList.add('hidden'); // Ocultar estado vacío
        if (historyTableBody.parentElement) {
            historyTableBody.parentElement.classList.remove('hidden'); // Mostrar <table>
        }
        ...
        historyData.forEach(entry => {
            ...
            // Sanitizar valores para evitar 'undefined' o 'null'
            ...
            // Renderizar la fila en el orden solicitado
            row.innerHTML = `
